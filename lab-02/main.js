...
function crearAnillo() {
  ...
  const count = Math.max(80, Math.round(parametros.columnas * 24));
  ...
  const material = new THREE.PointsMaterial({
    size: THREE.MathUtils.clamp(0.18 - parametros.separacion * 0.05, 0.06, 0.16),
    vertexColors: true,
    transparent: true,
    opacity: 1,
    depthWrite: false,
...
function actualizarAnillo(tiempo) {
  ...
  const mediaRR = (inputA + inputB) / 2;
  const baseRadius = THREE.MathUtils.mapLinear(mediaRR, 600, 1000, 3.3, 5.1);
  const respiracion = Math.sin(tiempo * parametros.frecuencia * Math.PI * 2) *
  ...
  for (let indice = 0; indice < cantidad; indice++) {
    ...
    const onda = Math.sin(angulo * 7 + tiempo * parametros.frecuencia * 0.8);
    ...
    const ruidoRadial = modoAutomatico && parametros.aleatoriedad > 0
      ...
      : onda * 0.2;
    const radio = baseRadius * (1 + respiracion) + ruidoRadial * dispersion;
